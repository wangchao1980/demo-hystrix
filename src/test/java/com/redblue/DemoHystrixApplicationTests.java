...
class DemoHystrixApplicationTests {
	...
	// @Test
	public void girlGirl() throws Exception {
		MvcResult mvcResult = mockMvc.perform(get("/girlGirl")).andExpect(status().isOk()).andReturn();
	...
	//@Test
	public void getOneGirl() throws Exception {
		MvcResult mvcResult = mockMvc.perform(get("/getOneGirl")).andExpect(status().isOk()).andReturn();
		log.info(mvcResult.getResponse().getContentAsString());
		String result = mvcResult.getResponse().getContentAsString();
